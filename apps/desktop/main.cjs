...
const onlineDashboard = "https://silvathiagoferreira.github.io/whoop-mg-platform/";
let agentProcess;
...
function readAccount() {
  try {
    ...
    const encoded = stored.encrypted ? safeStorage.decryptString(Buffer.from(stored.encrypted, "base64")) : stored.payload;
...
function writeAccount(account) {
  ...
  if (safeStorage.isEncryptionAvailable()) {
    fs.writeFileSync(accountPath, JSON.stringify({ encrypted: safeStorage.encryptString(payload).toString("base64") }));
  } else {
...
function publicAccount(account) {
  ...
  return { email: account.email, name: account.name, googleSub: account.googleSub || null, provider: account.provider, hasPassword: Boolean(account.passwordHash) };
}
...
function validateCredentials(email, password) {
  if (!/^\S+@\S+\.\S+$/.test(String(email || ""))) throw new Error("EMAIL_INVALID");
...
function startGoogleLogin() {
  ...
  return new Promise((resolve, reject) => {
    ...
    googleAuthServer = http.createServer((request, response) => {
      ...
      if (request.method !== "POST" || !request.url?.startsWith("/oauth/callback")) {
    ...
    timeout = setTimeout(() => finish(new Error("GOOGLE_LOGIN_TIMEOUT")), 5 * 60 * 1000);
...
async function resolveGoogleAccount(accessToken) {
  ...
  if (existing && existing.email !== user.email) throw new Error("ACCOUNT_MISMATCH");
  const account = {
...
app.whenReady().then(() => {
  ...
  ipcMain.handle("local-sign-in", (_event, payload = {}) => {
    ...
    if (!existing) {
      ...
      const account = { email, name: email.split("@")[0] || "Athlete", provider: "local", ...hashed, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
      writeAccount(account);
      return publicAccount(account);
    }
    if (existing.email !== email || !sameHash(password, existing)) throw new Error("CREDENTIALS_INVALID");
  ...
  ipcMain.handle("set-password", (_event, payload = {}) => {
    ...
    if (account?.passwordHash && !sameHash(String(payload.currentPassword || ""), account)) throw new Error("CURRENT_PASSWORD_INVALID");
    const hashed = hashPassword(password);
    const next = { ...account, ...hashed, updatedAt: new Date().toISOString() };
    writeAccount(next);
    return publicAccount(next);
  });
  ipcMain.handle("google-sign-in", async () => resolveGoogleAccount(await startGoogleLogin()));
  startLocalAgent();
  createWindow();
  app.on("activate", () => {
