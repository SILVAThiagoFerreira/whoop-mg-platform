...
function readSession() {
  try { return JSON.parse(localStorage.getItem("whoop-local-session") || "null"); } catch { return null; }
}

function saveSession(email) {
  try { localStorage.setItem("whoop-local-session", JSON.stringify({ email, name: email.split("@")[0] || "member" })); } catch { /* current-window session still works */ }
}
...
function updateScore(name, value, detailSelector, captionSelector, progressSelector, max) {
  const number = numericValue(value);
  setText(`#dash-${name}`, value);
  setText(detailSelector, value);
  setText(captionSelector, number === null ? "No data yet" : "Updated from local memory");
  const bar = document.querySelector(progressSelector);
  if (bar) bar.style.width = number === null ? "0%" : `${Math.min(100, Math.max(0, (number / max) * 100))}%`;
}
...
async function checkHealth() {
  ...
  try {
    ...
    setText("#observation-count", Number(body.observation_count || 0).toLocaleString("en-US"));
    setText("#chat-subtitle", `${body.observation_count || 0} observations · local memory`);
    const deviceLive = document.querySelector(".device-live");
    if (body.device_count > 0) {
      setText("#sidebar-device", `${body.device_count} DEVICE${body.device_count === 1 ? "" : "S"}`);
...
async function loadDashboard() {
  try {
    ...
    updateScore("recovery", recovery, "#recovery-detail-score", "#recovery-caption", ".recovery-card .score-progress i", 100);
    updateScore("strain", strain, "#strain-detail-score", "#strain-caption", ".strain-card .score-progress i", 21);
    updateScore("sleep", sleep, "#sleep-detail-score", "#sleep-caption", ".sleep-card .score-progress i", 100);
    setText("#dash-hrv", metricValue(snapshot, "hrv"));
    setText("#dash-rhr", metricValue(snapshot, "rhr"));
    setText("#sync-google", snapshot.google_sync_configured ? "ready" : "not configured");
    setText("#sync-ble", snapshot.ble_offload_status === "ready" ? "ready" : "not implemented");
...
async function askCoach(content) {
  ...
  try {
    const body = await getJson("/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: content, history }) });
...
document.querySelector("#scan")?.addEventListener("click", async (event) => {
  ...
  if (result) result.textContent = "Listening for BLE broadcasts for a few seconds…";
  try {
    const body = await getJson("/scan", { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" });
    if (!body.devices?.length) result.textContent = "No device found. Keep WHOOP nearby and try again.";
    else result.textContent = `${body.devices.length} device(s) found: ${body.devices.map((device) => device.name || device.address).join(", ")}`;
...
document.querySelector("#sync-now")?.addEventListener("click", async (event) => {
  ...
  try {
    const body = await getJson("/sync/google", { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" });
    result.textContent = body.result?.status === "NO_CHANGES" ? "Your copy is already up to date." : "Auxiliary copy updated.";
    await loadDashboard();
  } catch (error) {
...
document.querySelector("#logout")?.addEventListener("click", () => {
  try { localStorage.removeItem("whoop-local-session"); } catch { /* ignore */ }
  showLogin();
});
